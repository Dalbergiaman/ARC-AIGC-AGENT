    ...
    <div className="flex flex-col gap-4 px-4 py-4">
      {annotatedImage && (
        <div className="flex gap-3 rounded-lg border border-black/8 bg-white p-3">
          ...
          <button
            type="button"
            onClick={() => removeAnnotatedImage(sessionId)}
            ...
            return (
              <div key={item.taskId} className="overflow-hidden rounded-lg border border-black/8 bg-white">
                <button
                  ...
                  className="block h-44 w-full bg-black/[0.04] text-left disabled:cursor-default"
                >
                  {imageUrl ? (
                    /* eslint-disable-next-line @next/next/no-img-element */
                    <img src={imageUrl} alt="生成结果" className="size-full object-cover" />
                  ) : failed ? (
                    <div className="flex size-full items-center justify-center px-4 text-center text-sm text-red-500">
                      生成失败
                    </div>
                  ) : (
                ...
                <div className="flex flex-col gap-2 border-t border-black/6 px-3 py-2">
                  <div className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <span className="shrink-0 rounded bg-black/[0.04] px-1.5 py-0.5 text-[11px] text-muted-foreground">
                      {status}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    {item.provider && <span>{item.provider}</span>}
                    {typeof item.score === "number" && <span>评分 {item.score.toFixed(2)}</span>}
                  </div>
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    <button
                      type="button"
                      disabled
                      className="inline-flex items-center justify-center rounded-md border border-black/8 py-1.5 text-muted-foreground opacity-40"
                      title="D 阶段后启用"
                    >
