...
前端拿到 `url` 用于预览，图片此时只是 `uploaded` 草稿，不进入 AgentState，也不触发主对话。用户必须点击每张图片卡片上的「发送」按钮，图片才作为一轮正式消息提交给 Agent；普通文字发送不再自动夹带未发送图片。发送 payload 示例：
...
- [x] 图片上传与发送解耦：上传只保存文件和前端草稿；每张 control/reference 图片卡片独立点击发送后才作为正式消息进入 Agent 主链路
- [x] `agent_node` 对本轮显式发送图片调用 VLM 主对话，control image 回复底图优化建议，reference image 按 intent 回复如何参考；未指定方向时做全图简要分析
- [x] `enhance_prompt` 对参考图改为按 intent 克制注入结构化分析，保留生成稳定性但避免重复放大主对话分析
...
- 2026-05-13：明确并修正图像生成负向提示词的统一生效策略。此前 `enhance_prompt()` 会生成 `negative_prompt`，业务链路也会保存到 workspace / generation_tasks / 图库，但三个图像 provider 适配层都只使用 `request.prompt`，导致负向词没有实际参与生图。现统一不向任何 provider 发送独立 `negative_prompt` 字段，避免不同 API endpoint 字段支持差异导致请求失败；在 `core/image` 层用统一 helper 将负向约束拼到主 prompt 末尾（`避免出现以下问题：...`），确保百炼、火山、GrsAI 都能把负向约束作为文本指令传给图像模型。保留 `GenerationRequest.negative_prompt` 作为记录、展示和 refine 输入，避免丢失可观测性。
- 2026-05-13：优化图像生成长流程的前端可见性和 Agent JSON 回复展示时机。此前进入生成阶段后，前端只能看到主 Agent 原始 JSON 流，且必须等整条 LangGraph 流程 done 后才提取 `reply`；图像生成耗时期间用户不知道 Agent 在做什么。现 `streaming.py` 新增 `agent_reply` SSE：一旦从 Agent JSON 中解析出 `reply`，立即发送给前端替换当前助手消息，不再等生成/评估结束；新增 `agent_status` SSE，节点开始/结束和工具 start/end 会在对话栏当前助手消息下追加轻量状态日志（整理提示词、提交生成任务、等待结果、评估结果、重试优化等）。前端 `useSSE` / `chatStore` / `MessageList` 已接入状态列表，仍保留最终 done 时的兜底 reply 提取与落库。
- 2026-05-13：重构生成图自动评估与自优化策略，降低随机劣化。此前评分偏“需求字段符合度”，缺少第一眼画面质量门槛；`refine_prompt` 又只追最低分单项，容易为了材质/风格局部指标随机改坏整体。现评估维度改为：第一眼整体质量（透视/结构/错误内容/大面积单调）为最高权重，另含构图与视角、色彩与光影、建筑细节可信度、需求符合度、参考一致性；评估 JSON 新增 `fatal_issues` 和 `improvement_focus`。自优化 prompt 改为优先修严重问题与整体画面质量，明确保守修正、禁止随机换建筑类型/大构图/无关元素。自动重试门槛从 `score < 0.8` 调整为 `fatal_issues` 非空或 `score < 0.72`，避免中等可用图因轻微低分被反复随机重绘。
- 2026-05-13：修正 `workspace.llm_description` 组合时的近义重复。此前 `_compose_llm_description()` 先按 `design_state` 拼结构化字段句，再无条件追加 `agent_node` 返回的自然语言 `llm_description`，只能去掉完全包含的重复，无法识别“温润新中式住宅建筑效果图 / 温润新中式住宅建筑...”这类近义重复。现改为 LLM 整理描述优先，结构化字段只补充未被该描述覆盖的 building_type/style/material/lighting/viewpoint/surroundings/special_requirements；图生图说明仍作为独立上下文保留。新增回归测试覆盖“米白真石漆/黄色真石漆/人视角”不重复。
- 2026-05-13：火山引擎豆包图像生成请求默认关闭水印。官方 images generations payload 支持 `watermark` boolean 字段，默认 `true` 会添加“AI生成”水印；现 `backend/core/image/volcengine_client.py` 固定写入 `"watermark": false`，不改变其他 provider。`backend/tests/core/image/test_provider_payloads.py` 增加断言，确保单图/多图 payload 都携带该字段。
- 2026-05-13：图像生成 prompt 语言策略改为“默认中文，nano-banana 保持英文”。原因：当前百炼万相、火山即梦、GrsAI gpt-image 对中文建筑描述和图生图修改指令都较友好，中文能更完整保留用户原话中的位置、数量、材质、颜色、禁止项和编辑动作；统一转英文会造成信息压缩和翻译损失。实现上新增 `resolve_prompt_language(image_model)`，除模型名包含 `nano-banana` 外均返回 `zh`；`enhance_prompt_system()` / `refine_prompt_system()` 接收 `prompt_language`，中文模式输出中文正向/负向提示词，质量负向词允许保留 `blurry/distorted/watermark` 等英文关键词；Graph 从 Dashboard 当前 `image_provider.model` 推断语言并传入 enhance/refine。
- 2026-05-13：修正图生图/批注图再次修改时 prompt 信息损失问题。根因是 `enhance_prompt` 只接收结构化 `design_state`、参考图分析和 workspace 草稿，缺少用户最新原始修改要求、`control_image.note` 与 `annotated_image.note`，且 `enhance_prompt_system` 要求 150 词英文输出，容易把具体位置/数量/禁止项压缩成泛化风格词；生成前虽然会拼接图位说明，但核心英文 prompt 已经先被改写。现调整为 `enhance_prompt` 显式接收 latest_user_request / control_image / annotated_image，上下文中标明图生图是编辑基准、批注图是上一版修改指令，并把上限放宽到 220 词；`generate_image` 的图位前缀同步强化为“主约束图/允许重绘相关区域”，避免模型只做小修小改。
- 2026-05-13：调整主对话 `agent_system()` 的 `reply` 风格。此前主对话容易退化为"好的/已记录"式确认或表单式追问，用户体验单调。现将 `reply` 明确为"帮助用户打开下一步设计思考"：少复述用户已给方向，多围绕 1 个自然下一步做 2～3 个带画面感和设计理由的发散引导；按用户本轮修改的视角、材质、光线、风格、参考图等选择相邻议题；用户明确要求生成时不继续发散。新增边界：灵感建议只能写在 `reply`，不能擅自进入 `design_state_updates`，只有用户明确采纳才更新结构化状态。新增 `backend/tests/agent/test_agent_prompt.py` 防止该规则被回退。
- 2026-05-13：中栏生成图支持点击放大预览。`frontend/components/chat/MessageList.tsx` 为 anchored / unanchored 生成图缩略图增加按钮交互，点击后在全屏遮罩中以 `object-contain` 展示大图，支持点击背景、右上角关闭按钮和 Escape 关闭；图像 URL 继续复用 `resolveImageUrl()`，保证 `/static/generated/...` 走后端 API base URL。该改动仅影响中栏展示，不改变右侧生成图片区、批注、下载或生成流程。
- 2026-05-13：按阻塞耗时优化策略关闭低价值 thinking。保持主 Agent 对话理解 `agent_node` 与失败图 `refine_prompt()` 默认 thinking，以保留多轮意图判断和基于失败图修正的推理能力；显式关闭高频/结构化/评估类调用的 thinking：参考图分析 `analyze_reference_image()`、RAG 氛围分析 `ambience_rag_image`、prompt 初稿生成 `enhance_prompt()`、图像评估 `evaluate_generated_image()`（含 JSON 重试）、image-rag-mcp 图库 caption `generate_caption()`。其中后端统一通过 `LLMClient.ainvoke(..., enable_thinking=False)` 传递；image-rag-mcp 直连 HTTP 时按 provider 写入百炼 `enable_thinking: false` 或火山 `thinking: {"type": "disabled"}`。会话标题生成此前已关闭，保持不变。
- 2026-05-13：补齐 VLM 调用的推理模式开关。此前 `LLMClient.ainvoke(..., enable_thinking=False)` 只对纯文本调用生效；一旦传 `images` 就走 `ainvoke_with_vision()`，底层不会透传关闭 thinking 参数，导致视觉分析、图像评估、失败图 refine 等 VLM 调用只能使用 provider/model 默认推理模式。现将 `enable_thinking` 加到 `LLMClientBase.ainvoke_with_vision()`、`_BaseHTTPLLMClient.ainvoke_with_vision()` 和 `LLMClient.ainvoke()` 的 vision 分支：调用方显式传 `enable_thinking=False` 时，百炼 payload 会带 `enable_thinking: false`，火山 payload 会带 `thinking: {"type": "disabled"}`；默认值仍为 `True`，保持既有行为不变。新增 `backend/tests/core/llm/test_http_client_thinking.py` 和 wrapper 测试覆盖。
- 2026-05-13：自动重试时把上一轮低分生成图加入图生图输入。`generate_image()` 新增第四类语义槽位 `failed_image`（来源于 `_current_gen_result.image_url`），只在重试轮次存在；图片输入仍按实际非空列表动态编号：control / annotated / rag / failed 依序过滤空值后生成 `input_image_urls` 与 `图N` 说明，避免 UI/业务槽位空洞导致 prompt 图号和 provider 实际图序错位。例如无 annotated 时，RAG 是图2、failed 是图3；四类都存在时 failed 才是图4。失败图说明明确为“上一轮低分生成结果，仅用于识别需要修正的问题；不要复制其中错误内容”，并强调优先以结构底图保留体量、透视、窗户和构图，防止失败图覆盖原始底图。
- 2026-05-13：修正自动重试缺少视觉闭环的问题。此前 `evaluate_generated_image()` 会看生成图并输出分数/feedback，但 `refine_prompt()` 只拿到原 prompt 与文字评估，不看低分图本身，导致自动重试更像重新随机生成，缺少针对失败图的具体修正。现将 `_current_gen_result.image_url` 从 `refine_prompt_node` 传入 `refine_prompt()`，并用 `_to_data_url()` 把 `/static/generated/...` 等本地 URL 转成 data URL 随 VLM 请求发送；`refine_prompt_system()` 在有失败图时明确要求结合图像判断哪些内容应保留、哪些与提示词/评估不一致，图生图任务重点检查体量、透视、窗户和构图。该改动只让“修 prompt 的 VLM”看到失败图；是否把失败图继续作为下一轮图生图输入仍取决于各 provider 多图/图生图能力，暂不扩大范围。
- 2026-05-13：修正 `workspace.prompt_draft.llm_description` 语义污染问题。此前 `_compose_llm_description()` 会把“画面主体清晰、结构完整”“外立面材质和色彩需要写得更具体”“补充明确的光线方向”等质量检查/系统指导句写入 `llm_description`，下一轮又通过 `workspace` 回传给 Agent，导致字段不断累加，像系统提示词而不是画面描述；`enhance_prompt_node` / `refine_prompt_node` 还会把最终生成 prompt 反写到 `llm_description`，进一步污染草稿。现调整为：`llm_description` 只保存面向用户可读的画面内容描述（建筑类型、风格、材质、色彩、光线、视角、环境、特殊要求），去重后输出；生成/修正节点只更新 `negative_prompt`，不再用 `enhanced.prompt` / `refined.prompt` 覆盖 `llm_description`。新增 `backend/tests/agent/test_prompt_draft.py` 覆盖，防止“避免过于笼统/补充明确/画面主体清晰”等元提示再次进入该字段。
- 2026-05-13：D-4 联调中修正图库图片可见性。问题：D-3 将 PG/Milvus 的 `image_url` 存为 `/static/library/{id}.{ext}`，前端经 FastAPI 静态挂载能展示，但在 Attu/Milvus 管理界面中是相对路径，无法直接打开。现新增独立开关 `IMAGE_LIBRARY_STORAGE=local|minio`：默认保持 local；设为 minio 时，`image-rag-mcp/core/storage.py` 在存图库时仍写本地 `library_images/` 副本用于 VLM caption、image embedding 和 `search_by_image` 本地反解，同时用轻量 SigV4 S3 请求把对象上传到业务 MinIO bucket `image-library`，设置 bucket 公开读，并将 PG/Milvus `image_url` 写为 `IMAGE_LIBRARY_MINIO_PUBLIC_ENDPOINT/bucket/key`。`docker-compose.yml` 暴露业务 MinIO API 端口 `9000:9000`，后端 `_build_mcp_client()` 显式把图库 MinIO 配置透传给 MCP 子进程，避免 `.env` 只被 FastAPI 读取而 MCP 不生效。该改动不改变通用业务文件存储：`backend/services/storage_service.py` 的 `STORAGE=minio` 分支仍未实现。
