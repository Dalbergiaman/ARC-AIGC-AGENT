...
- [ ] 编写 `components/workspace/WorkspacePanel.tsx`，提供"提示词与参考图"、"生成图片"两个标签页
- [ ] 编写 `PromptReferenceTab.tsx`：顶部 prompt / negative prompt 展示与手动编辑
- [ ] 编写 `ImageUploader.tsx` + `ReferenceIntentPicker.tsx`：上传参考图后必须选择参考意图（构图 / 色彩 / 建筑样式 / 材质 / 光线 / 环境 / 其他）和可选说明
- [ ] 扩展 `POST /api/chat/sessions/{session_id}/messages` 请求体，支持 `reference_images` 与 `workspace` payload
- [ ] 后端将 `intent` / `note` 写入 `ReferenceImageAnalysis.reference_intent` / `intent_note`
- [ ] 验证 Agent 收到图片后触发 `analyze_reference_image`，并保留用户标注的参考意图
...
- [ ] 如右侧 prompt 需要稳定实时刷新，后端补充 SSE `prompt_update` 事件；否则 E-3 先以用户草稿 + 生成流程结果展示为准
- [ ] 验证用户编辑 prompt / 参数 / 风格后，下一轮 Agent 能在上下文中读取这些内容
