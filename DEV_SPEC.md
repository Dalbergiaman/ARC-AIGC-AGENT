...
每次工具调用完成后都回到 `agent` 节点重新决策，真实路径示例：

```
用户消息 → agent → enhance_prompt → agent → generate_image
       → agent → evaluate_generated_image → agent → refine_prompt
       → agent → generate_image → agent → evaluate_generated_image
       → agent → 回复用户
```
...
- 每轮对话进入 `agent` 节点时立即 reset `cancel_event`，确保新一轮生成不被上一轮的中断信号误触发。
...
| 用户中断 | `cancel_event` + LangGraph `interrupt_before` | 每轮 agent 节点前及轮询内均可中断 | 不计入 `retry_count`，Agent 回复"已中断"并等待下一条消息 |
...
**`generate_image` 轮询内的中断**：使用内存 `cancel_event` 字典（`{session_id: asyncio.Event}`），FastAPI 收到新消息时调 `cancel(session_id)` set 该 event，`generate_image` 轮询每次 `asyncio.sleep(2)` 后检查 `event.is_set()`，检测到后抛 `CancelledError` 提前退出，回到 `agent` 节点感知新消息。每轮对话进入 `agent` 节点时调 `reset(session_id)` 清除信号，避免误触发后续生成。

session_id 在整个对话窗口内不变，`cancel_event` 复用同一个 key，与 LangGraph checkpointer 的对话历史完全独立。
...
# 2. 轮询 Redis，每 2s 检查一次，最多等 120s；同时检查 cancel_event
cancel_event = cancel_registry.get_or_create(session_id)
for _ in range(60):
...
- 2026-05-04：确认 Agent 流程设计：每次工具调用后回到 agent 节点重新决策（标准 ReAct）；store_to_library 解耦出 Agent，改为前端按钮触发独立 REST 接口 POST /api/library/store；generate_image 轮询内通过内存 cancel_event 字典支持用户随时中断，session_id 贯穿整个对话窗口不变，每轮 agent 节点入口 reset cancel_event。
