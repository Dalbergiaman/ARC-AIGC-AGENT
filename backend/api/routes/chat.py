...
class AnnotatedImagePayload(BaseModel):
    file_id: str
    url: str
    note: str = ""
...
class MessageRequest(BaseModel):
    ...
    annotated_image: AnnotatedImagePayload | None = None
...
async def _generate_sse(
    ...
    try:
        ...
        annotated_image_raw = await r.get(f"annotated_image:{session_id}:{stream_id}")
        ...
        if annotated_image_raw:
            ...
            if annotated_url.startswith("/"):
                annotated_url = f"{base}{annotated_url}"
            input_state["annotated_image"] = {
                "file_id": annotated_image.get("file_id", ""),
                "image_url": annotated_url,
                "note": annotated_image.get("note", ""),
                "sent": True,
            }
        if workspace_raw:
        ...
        turn_metadata = {
            ...
            "has_annotated_image": bool(annotated_image_raw),
        ...
        with start_observation(
            ...
            async for chunk in stream_agent_events(graph, config, input_state):
                ...
                # still-running generate_image span as an error in Langfuse.
