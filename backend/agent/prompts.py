...
def enhance_prompt_system(
    ...
    image_edit_prompt_structure = (
        "【图生图建筑摄影提示词结构】\n"
        "当存在图生图编辑上下文时，prompt 按以下顺序组织，不要把这些规则逐字照抄成标题：\n"
        "1. 编辑任务声明：先说明将输入图转为真实建筑摄影/商业级建筑可视化/照片级渲染，而不是重新设计一张无关图。\n"
        "2. 结构保持：明确保持原图拍摄角度、镜头高度、广角透视、主体建筑比例、体量关系、空间关系、立面分区、开窗节奏；"
        "如果原图有道路、斑马线、水景、树木轮廓、车辆或行人位置，除非用户要求改变，否则只做真实化和细节增强。\n"
        "3. 材质细化：把材质写成可执行的物理描述，例如玻璃幕墙的透明度、反射天空/绿植、室内可见度与灯光层次，"
        "清水混凝土的细腻肌理，金属/木材/石材/织物的粗糙度、边缘和节点细节。\n"
        "4. 光照天气：明确天空、天气、光线时段、直射或漫射、色温、明暗对比和阴影软硬；光照必须统一，不能只写电影感。\n"
        "5. 环境配景：人物、车辆、树木、草坪、水面、铺装、湿地植物等只在用户要求或原图存在时适度增强，"
        "用于尺度和氛围；数量少而准，不能抢主体。\n"
        "6. 摄影后期：用低饱和、清透、淡冷蓝、偏白天空、暖调中性色、柔和漫射或黄金时刻等具体色彩方向，"
        "少量加入建筑摄影、电影感、商业级景观渲染等风格目标，但必须服务于前面的结构、材质和光线。"
    )
...
- 如果存在"图生图编辑上下文"，prompt 必须优先保留原图视角、构图、主体建筑比例、体量关系、空间关系、立面节奏和主要场地关系；用户明确写"严格保持"时，还要保留前景、道路、水景、车辆、行人和绿化轮廓等可识别元素
...
- prompt 必须把"高级质感"落到具体画面控制：画面克制、主体清晰、玻璃反射受控、能看到适度室内结构和灯光层次、材质真实但不过度、环境元素少而准；不要只堆"大师级/高质量/超细节"等泛化形容词
- prompt 中的真实感必须落到物理关系：统一光源、合理反射、真实植物、人物尺度、车辆不抢主体、地面或水面倒影干净且受控
- 色调描述必须具体：例如淡冷蓝、低饱和、偏白天空、暖调中性色、柔和漫射光、黄金时刻暖光等；不要只写"高级调色"
- negative_prompt 必须包含品质护栏中的杂乱感和低质问题：杂乱前景、随机车辆、车辆喧宾夺主、主体建筑被重构、原图视角改变、立面节奏错乱、脏玻璃反射、玻璃过曝死白、碎片化反光、植被噪声、水面/地面反射脏乱、学生作业感等；若 prompt 为中文，negative_prompt 仍可混用英文质量词
- 不要在 prompt 中重复相同概念"""
